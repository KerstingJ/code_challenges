...
function hourglassSum(arr) {
    ...
    const getHourglassSum = (arr, row, col) => {

        // Top of the Hourglass
        let topSum = arr[row][col] + arr[row][col+1] + arr[row][col+2]

        // The Funnel
        let middleSum = arr[row+1][col+1]

        // The Base
        let bottomSum = arr[row+2][col] + arr[row+2][col+1] + arr[row+2][col+2]

        // return arr[row][col]
        return topSum + middleSum + bottomSum
    }

    // we need to iterate over the 2d array
    // skipping over the last 2 rows and columns
    // each coord (row, col) is going to be the top left of an hour glass
    for (let row = 0; row < arr.length - 2; row++){
        for (let col = 0; col < arr[row].length - 2; col++){
            let sum = getHourglassSum(arr, row, col);
            if (sum > maxSum) {
                maxSum = sum;
            }
        }
